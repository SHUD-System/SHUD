...
design.md D9 trigger #2 requires `S6b.2 = "审查为'无修改'" 跳过 fix` with a SIGNED conclusion. This trigger is **NOT satisfied by an unsigned evidence pack**. D9 fast-path (B1a / B1b merge into `B1-tag`) is **gated** until PI sign-off arrives. S6c (#188-#190) proceeds with separate B1a-tag and B1b-tag (D9 fast-path not satisfied) — both subsequently subject to D11 force-update prohibition.
