...
//    isf = uYsf[i] - qEleInfil[i] + qEleExfil[i];
    ...
    for (j = 0; j < 3; j++) {
        ...
        if(ilake >= 0){  /* For Lake element */
            nsf = yLakeStg[ilake];
            nsf = nsf < 0. ? 0. : nsf;
            dh = (isf + Ele[i].zmax) - (nsf + lake[ilake].zmin);
            Ymean = avgY_sf(Ele[i].zmax, isf, lake[ilake].zmin, nsf, Ele[i].depression);
            if(dh > isf){
                dh = isf;
                Ymean = isf;
            }else{
            ...
            if(dh < 0){
                printf("Warning: Lake(%d) Flood to Element (%d) at time(%f day)!\n", ilake+1, i+1, t/1440);
            }
            if(Ymean <= 0.){
                Q = 0.;
            }else{
